...
const LATEST_API = "https://macaumarksix.com/api/macaujc3.com";
const HISTORY_API_PREFIX = "https://history.macaumarksix.com/history/macaujc3/expect/";
...
const PING_MODELS = [
  // 重点改动：平特一肖不再重押“跟随关系”，改成近30期高频 + 近10期热度为主
...
function onlyDigits(text) { const s = String(text || ""); if (!s) return false; for (const ch of s) if (!isDigit(ch)) return false; return true; }
function isSeparator(ch) { if (!ch) return true; const code = ch.charCodeAt(0); if (code <= 32 || code === 12288) return true; return "，、。；;|/,+:-_()[]{}".includes(ch); }
function splitItems(value) {
  const text = String(value || ""); const out = []; let buf = "";
  for (const ch of text) { if (isSeparator(ch)) { if (buf.trim()) out.push(buf.trim()); buf = ""; } else buf += ch; }
  if (buf.trim()) out.push(buf.trim()); return out;
}
function extractPairs(text) {
  const body = String(text || ""); const pairs = []; let i = 0;
  while (i < body.length) {
    if (!isDigit(body[i])) { i += 1; continue; }
    let num = ""; while (i < body.length && isDigit(body[i])) { num += body[i]; i += 1; }
    while (i < body.length && isSeparator(body[i])) i += 1;
    const zodiac = simpleZodiac(body[i]); const n = Number(num);
    if (n >= 1 && n <= 49 && ZODIACS.includes(zodiac)) { pairs.push([String(n).padStart(2, "0"), zodiac]); i += 1; }
  }
  return pairs;
}
function normalizeRecord(row) {
  ...
  return { expect: String(row.expect || row.issue || row.period || row.qihao || ""), openTime: String(row.openTime || row.open_time || row.time || ""), openCode: codeList.join(","), zodiac: zodiacList.join(","), codeList, zodiacList };
}
function parseRows(text) {
  ...
  for (const line of raw.split(String.fromCharCode(10)).map((x) => x.trim()).filter(Boolean)) {
    const parts = splitItems(line); if (parts.length < 8) continue;
    const expect = parts[0]; if (!onlyDigits(expect) || expect.length < 5) continue;
    const pairs = extractPairs(parts.slice(1).join(" "));
    if (pairs.length >= 7) rows.push(normalizeRecord({ expect, openCode: pairs.slice(0, 7).map((p) => p[0]).join(","), zodiac: pairs.slice(0, 7).map((p) => p[1]).join(",") }));
  }
  return rows;
}
...
function uniqRows(rows) { const map = new Map(); for (const row of rows) if (row.expect) map.set(row.expect, row); return [...map.values()].sort((a, b) => String(b.expect).localeCompare(String(a.expect))); }
function chronological(rows) { return [...uniqRows(rows)].sort((a, b) => String(a.expect).localeCompare(String(b.expect))); }
function latestRows(rows, n = WINDOW_SIZE) { return uniqRows(rows).slice(0, n); }
function nextExpect(expect) { const s = String(expect || ""); if (!onlyDigits(s)) return ""; return String(Number(s) + 1).padStart(s.length, "0"); }
function round(n) { return Math.round(n * 100) / 100; }
function pct(a, b) { return b ? Math.round((a / b) * 100) : 0; }
function clamp(n, min, max) { return Math.min(max, Math.max(min, n)); }
function add(obj, key, value) { obj[key] = (obj[key] || 0) + value; }
function ballColor(num) { if (RED.has(num)) return "红波"; if (BLUE.has(num)) return "蓝波"; if (GREEN.has(num)) return "绿波"; return ""; }
function bigSmall(num) { return Number(num) >= 25 ? "大" : "小"; }
function oddEven(num) { return Number(num) % 2 === 1 ? "单" : "双"; }
function specialNum(row) { return row && row.codeList ? row.codeList[6] : ""; }
function headOfNum(num) {
  const n = Number(num);
  if (!n || n < 1 || n > 49) return "";
  return `${Math.floor(n / 10)}头`;
}
function classOfRow(row, type) { const n = specialNum(row); if (!n) return ""; if (type === "color") return ballColor(n); if (type === "size") return bigSmall(n); return oddEven(n); }
function baseRatings(models) { return Object.fromEntries(models.map((m) => [m.id, 1])); }
function cleanRatings(ratings, models) { const out = baseRatings(models); for (const m of models) out[m.id] = clamp(Number(ratings && ratings[m.id] ? ratings[m.id] : 1), 0.25, 5); return out; }
...
async function fetchLatest30() {
  ...
  for (let e = latestExpect - 1; e >= latestExpect - WINDOW_SIZE + 1; e--) {
    try { rows.push(...await fetchExpect(e)); } catch (err) {}
  }
...
function scorePingModel(rows, model) {
  ...
  chrono.forEach((row, index) => {
    const seen = new Set(row.zodiacList); const age = chrono.length - 1 - index;
    for (const z of seen) if (stats[z]) { stats[z].issue += 1; if (age < 5) stats[z].recent5 += 1; if (age < 10) stats[z].recent10 += 1; }
    for (const z of row.zodiacList) if (stats[z]) stats[z].occur += 1;
    const sp = row.zodiacList[6]; if (stats[sp]) stats[sp].special += 1;
  });
  const half = Math.floor(chrono.length / 2); const older = chrono.slice(0, half); const newer = chrono.slice(half);
  for (const z of ZODIACS) { let oldHit = 0, newHit = 0; for (const row of older) if (row.zodiacList.includes(z)) oldHit += 1; for (const row of newer) if (row.zodiacList.includes(z)) newHit += 1; stats[z].trend = Math.max(0, newHit - oldHit); }
  const lastSpecial = latest[0] && latest[0].zodiacList[6] ? latest[0].zodiacList[6] : "";
  if (lastSpecial) for (let i = 1; i < chrono.length; i++) if (chrono[i - 1].zodiacList[6] === lastSpecial) { const seen = new Set(chrono[i].zodiacList); for (const z of seen) if (stats[z]) stats[z].follow += 1; }
...
function scorePing(rows, ratings) {
  ...
  for (const model of PING_MODELS) {
    const list = scorePingModel(rows, model); const max = Math.max(1, list[0]?.score || 1);
    list.forEach((item, index) => { scores[item.label] += ((item.score / max) * 100 + Math.max(0, 12 - index) * 3) * (ratings[model.id] || 1); });
  }
...
function scoreClassModel(rows, model, labels, type) {
  ...
  chrono.forEach((row, index) => { const v = classOfRow(row, type); if (!stats[v]) return; const age = chrono.length - 1 - index; stats[v].count += 1; if (age < 5) stats[v].recent5 += 1; if (age < 10) stats[v].recent10 += 1; });
  const latestValue = latest[0] ? classOfRow(latest[0], type) : "";
  if (latestValue && stats[latestValue]) for (const row of latest) { if (classOfRow(row, type) === latestValue) stats[latestValue].streak += 1; else break; }
  if (latestValue) for (let i = 1; i < chrono.length; i++) if (classOfRow(chrono[i - 1], type) === latestValue) { const next = classOfRow(chrono[i], type); if (stats[next]) stats[next].follow += 1; }
  const half = Math.floor(chrono.length / 2); const older = chrono.slice(0, half); const newer = chrono.slice(half);
  for (const label of labels) { let oldHit = 0, newHit = 0; for (const row of older) if (classOfRow(row, type) === label) oldHit += 1; for (const row of newer) if (classOfRow(row, type) === label) newHit += 1; stats[label].trend = Math.max(0, newHit - oldHit); }
...
function scoreHeadForKill(rows) {
  ...
  const stats = Object.fromEntries(HEADS.map((h) => [h, { label: h, count: 0, recent5: 0, recent10: 0, follow: 0, trend: 0, score: 0 }]));
  chrono.forEach((row, index) => {
...
function scoreSpecialZodiacForKill(rows) {
  const latest = latestRows(rows);
  const chrono = [...latest].reverse();
  const stats = Object.fromEntries(ZODIACS.map((z) => [z, { label: z, count: 0, recent5: 0, recent10: 0, follow: 0, trend: 0, score: 0 }]));
  chrono.forEach((row, index) => {
...
function scoreClass(rows, labels, type, ratings) {
  ...
  for (const model of CLASS_MODELS) {
    const list = scoreClassModel(rows, model, labels, type); const max = Math.max(1, list[0]?.score || 1);
    list.forEach((item, index) => { scores[item.label] += ((item.score / max) * 100 + Math.max(0, labels.length - index) * 5) * (ratings[model.id] || 1); });
  }
...
function updateRatings(pred, actual, pingRatings, classRatings) {
  ...
  const newClass = { color: { ...classRatings.color }, size: { ...classRatings.size }, parity: { ...classRatings.parity } };
  const actualSet = new Set(actual.zodiacList);
  const actualColor = classOfRow(actual, "color"); const actualSize = classOfRow(actual, "size"); const actualParity = classOfRow(actual, "parity");
  for (const model of PING_MODELS) newPing[model.id] = clamp((newPing[model.id] || 1) + (actualSet.has(pred.byPingModel?.[model.id]) ? 0.12 : -0.08), 0.25, 5);
  for (const model of CLASS_MODELS) {
...
function trainRatings(rows) {
  ...
  let classRatings = { color: baseRatings(CLASS_MODELS), size: baseRatings(CLASS_MODELS), parity: baseRatings(CLASS_MODELS) };
...
function settledHistory(rows, pingRatings, classRatings) {
  ...
  for (let i = Math.max(TRAIN_MIN, all.length - 10); i < all.length; i++) {
    ...
    out.unshift({
      ...
      actualSpecialZodiac,
      actualHead: headOfNum(specialNum(actual)),
      actualColor,
      actualSize,
      actualParity,
      pingHit: actualSet.has(pred.ping),
      killHeadHit: killHead && killHead !== headOfNum(specialNum(actual)),
...
function historyStats(history) {
  ...
  return { total, pingRate: pct(count("pingHit"), total), killHeadRate: pct(count("killHeadHit"), total), killRate: pct(count("killHit"), total), colorRate: pct(count("colorHit"), total), sizeRate: pct(count("sizeHit"), total), parityRate: pct(count("parityHit"), total) };
}

function Badge({ children }) { return <span className="inline-flex items-center rounded-full bg-slate-800 px-3 py-1 text-xs text-slate-300">{children}</span>; }
function ScoreBar({ value }) { const width = clamp(value / 500 * 100, 4, 100); return <div className="h-2 rounded-full bg-slate-800 overflow-hidden"><div className="h-full bg-slate-200 rounded-full" style={{ width: `${width}%` }} /></div>; }
function RatingRows({ title, models, ratings }) { return <div><h3 className="font-semibold mb-2">{title}</h3><div className="space-y-2">{models.map((m) => <div key={m.id} className="rounded-2xl bg-slate-950 border border-slate-800 p-3"><div className="flex items-center gap-3"><div className="w-28 text-sm">{m.name}</div><div className="flex-1"><ScoreBar value={(ratings[m.id] || 1) * 100} /></div><div className="w-14 text-right">{round(ratings[m.id] || 1)}</div></div></div>)}</div></div>; }
...
export default function ZodiacPredictionApp() {
  ...
  useEffect(() => {
    let saved = [];
    try { saved = JSON.parse(localStorage.getItem("zodiac_open_ready_records_v1") || "[]").map(normalizeRecord); } catch (e) {}
    setRecords(uniqRows([...saved, ...FALLBACK_ROWS.map(normalizeRecord)]));
    syncLatest30(true);
  }, []);
  useEffect(() => { localStorage.setItem("zodiac_open_ready_records_v1", JSON.stringify(records)); }, [records]);
  useEffect(() => {
  ...
  function importRows() {
    ...
    if (!rows.length) { setStatus("没有识别到有效数据"); return; }
    setRecords((old) => uniqRows([...old, ...rows]));
